...
void TestApp::initializeApp(int stage)
{
    ...
    // start our timer!
    timerMsg = new cMessage("TestApp Timer");
...
void TestApp::finishApp()
{
    ...
    globalStatistics->addStdDev("TestApp: Sent packets", numSent);
    globalStatistics->addStdDev("TestApp: Received packets", numReceived);
}


// handleTimerEvent is called when a timer event triggers
void TestApp::handleTimerEvent(cMessage* msg)
{
    // is this our timer?
    if (msg == timerMsg) {
        ...
        for (int i = 0; i < numToSend; i++) {
            ...
            TestMessage *myMessage; // the message we'll send
            myMessage = new TestMessage();
...
void TestApp::deliver(OverlayKey& key, cMessage* msg)
{
    // we are only expecting messages of type MyMessage, throw away any other
    TestMessage *myMsg = dynamic_cast<TestMessage*>(msg);
    if (myMsg == NULL) {
...
// Unknown packets can be safely deleted here.
void TestApp::handleUDPMessage(cMessage* msg)
{
